 ...
 * @return
 */
CK_RV generate_aes_keyXXX(CK_SESSION_HANDLE session,
    CK_ULONG key_length_bytes,
    CK_OBJECT_HANDLE_PTR key) {
    CK_RV rv;
    CK_MECHANISM mech;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    rv = pkcs11_initialize(p11SoName);
    ...
    static char* p11Pin = "12345678";

    rv = pkcs11_open_session(p11Pin, &session);
    if (CKR_OK != rv) {
        return rc;
    }
...
//    static char* wrappedKeyName = "wrapped.key";
    ...
    RSA* key = read_RSA_PUBKEY(pubKeyName);
    if (NULL == key) {
